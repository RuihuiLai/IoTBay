    ...
    public Customer findCustomer(String email, String password) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (customerEmail.equals(email) && customerPassword.equals(password)) {
                ...
                return new Customer(customerID, customerFirstName, customerLastName, customerEmail, customerPassword, customerGender, customerAddress, customerPhone, false);
            }
        }
        return null;
    }
...
//Add a user-data into the database   
    public void addCustomer(String firstName, String lastName, String eMail, String password, String gender, String address, String phoneNumber) throws SQLException {                   //code for add-operation       
        st.executeUpdate("INSERT INTO IOTUSER.\"USER\"(FIRSTNAME, LASTNAME, EMAIL, PASSWORD, GENDER, ADDRESS, PHONENUMBER) " + "VALUES('" + firstName + "','" + lastName + "','" + eMail + "','" + password + "','" + gender + "','" + address + "','" + phoneNumber + "')");
    }
...
//update a user details in the database   
    public void updateCustomer(String userID, String firstName, String lastName, String eMail, String password, String gender, String address, String phoneNumber) throws SQLException {
        ...
        st.executeUpdate("UPDATE IOTUSER.\"USER\" SET FIRSTNAME='" + firstName + "', LASTNAME='" + lastName + "',EMAIL='" + eMail + "',PASSWORD='" + password + "',GENDER='" + gender + "',ADDRESS='" + address + "',PHONENUMBER='" + phoneNumber + "' WHERE USERID=" + userID + "");
    }

//delete a user from the database   
    public void deleteCustomer(String userID) throws SQLException {
        //code for delete-operation   
        st.executeUpdate("DELETE FROM IOTUSER.\"USER\" WHERE USERID=" + userID + "");
    }

    public ArrayList<Customer> fetchCustomers() throws SQLException {
        ResultSet rs = st.executeQuery("SELECT * FROM IOTUSER.\"USER\"");
        ...
        while (rs.next()) {
            ...
            String customerAddress = rs.getString(7);
            String customerPhone = rs.getString(8);
            customers.add(new Customer(customerID, customerFirstName, customerLastName, customerEmail, customerPassword, customerGender, customerAddress, customerPhone, false));
        }
        return customers;
    }

    public boolean checkCustomer(String userID) throws SQLException {
        ResultSet rs = st.executeQuery("SELECT * FROM IOTUSER.\"USER\" WHERE USERID=" + userID + "");
        if (rs.next()) {
            return true;
        }
        return false;
